...
echo "; Config file for MAC" >> $1tmp1.xml
...
while IFS=, read number DisplayName password; do

		sed -e "/^fullname =/s:.*:fullname = $DisplayName:g" -e "/^secret =/s:.*:secret = $password:g" \
		    -e "/^[](def_setting)/s:.*:[$number](def_setting):g" < $1tmp4.xml >> "$1users.cfg"
done <$1tmp5.csv
